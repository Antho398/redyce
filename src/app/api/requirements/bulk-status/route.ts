/**
 * Route API pour modifier le statut de plusieurs exigences en lot
 * POST /api/requirements/bulk-status
 */
...
const bulkStatusSchema = z.object({
  ...
  status: z.enum(['A_TRAITER', 'COVERED', 'SUPPRIMEE']),
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Mettre à jour toutes les exigences
    ...
    return NextResponse.json<ApiResponse>(
      {
        success: false,
        error: {
          message: error instanceof Error ? error.message : 'Failed to update bulk status',
        },
      },
      { status: 500 }
    )
